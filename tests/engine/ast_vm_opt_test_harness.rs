...
use ast_vm::{opt, lang::LuaValue, stdlib};
use luar_syn::lua_parser;

macro_rules! run_lua_test {
    ($group_name: expr, $module_str: expr) => {
        $crate::ast_vm_opt_test_harness::run_lua_test_impl(module_path!(), $group_name, $module_str);
    };
}
...
pub fn run_lua_test_impl(module_path: &str, group_name: &str, module_str: &str) {
    ...
    let module = opt::compile_module(module);
